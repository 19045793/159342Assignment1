		...
		while (1)
		{
			...
			if (strncmp(receive_buffer, "RETR", 4) == 0)
			{
					...
					if(file_type == FileType::BINARY){
						file_to_fetch = fopen(file_name, "rb");
					}else{
						file_to_fetch = fopen(file_name, "r");
					}
					if(!file_to_fetch){
						count = snprintf(send_buffer,BUFFER_SIZE,"450 Requested file action not taken. File unavailable \r\n");	
						goto RETR_ERROR;
					}
					snprintf(send_buffer, BUFFER_SIZE, "125 Data connection already open; transfer starting.\r\n");
					if(count >=0 && count < BUFFER_SIZE){
					    bytes = send(ns, send_buffer, strlen(send_buffer), 0);
					}
					printf("[DEBUG INFO] <-- %s\n", send_buffer);
					...
					if (bytes < 0) break;
					send_buff = (char*)malloc(4096);
					while((bytes_read = fread(send_buff, 1, 4096, file_to_fetch))>0){
						bytes = send(s_data_act, send_buff, bytes_read, 0);
						if(bytes<bytes_read){
							break;
						}
					}
					count=snprintf(send_buffer,BUFFER_SIZE,"226 Closing data connection. Requested file action successful\r\n");
					#if defined __unix__ || defined __APPLE__
						close(s_data_act);
					#elif defined __WIN32__
			...
			if (strncmp(receive_buffer,"EPRT",4)==0)  {  //more work needs to be done here
					...
					char modePtr, *hostPtr, *portPtr;
					 int delims=0;
					 int y = strlen(receive_buffer);
					 for(int x=0;x<y;x++){
						if(receive_buffer[x]=='|'){
							delims++;
						}
						if(receive_buffer[x]=='|' && delims==1){
							//mode
							modePtr = receive_buffer[x+1];  
						}
						else if(receive_buffer[x]=='|' && delims==2){
							//host
							hostPtr = &receive_buffer[x+1];
							receive_buffer[x] = '\0';
						}
						else if(receive_buffer[x]=='|' && delims==3){
							//port
							portPtr = &receive_buffer[x+1];
							receive_buffer[x] = '\0';
						}
						else if(receive_buffer[x]=='|' && delims==4){
							//final delim
							receive_buffer[x] = '\0';
						}
					 }
					 void *vsockaddr;
					 if(modePtr == '2'){
						vsockaddr = malloc(sizeof(struct sockaddr_in6));
						memset(vsockaddr, 0, sizeof(struct sockaddr_in6));
						struct sockaddr_in6 *vsockaddr_v6 = (struct sockaddr_in6*) vsockaddr;
						vsockaddr_v6->sin6_family = AF_INET6;
						vsockaddr_v6->sin6_port = htons(atoi(portPtr));
						int inet_pton_rtn;
						#ifdef __WIN32__
							/*Im having to add this dynamic function resolution because MinGW on windows doesn't have inet_pton defined 
							*/
							int (*inet_pton_ptr)(INT, PCSTR, PVOID) = GetProcAddress(GetModuleHandle("Ws2_32.dll"), "inet_pton");
							inet_pton_rtn= (int)inet_pton_ptr(AF_INET6, hostPtr, &vsockaddr_v6->sin6_addr); 
						#endif
						#if defined __unix__ || defined __APPLE__
							inet_pton_rtn = inet_pton(AF_INET6, hostPtr, &vsockaddr_v6->sin6_addr);
						#endif 
						printf("[DEBUG] value of inet_pton_rtr %d\n", inet_pton_rtn);
						if(inet_pton_rtn!=1){
							break;
						}
						s_data_act = socket(vsockaddr_v6->sin6_family, SOCK_STREAM, 0);
						if(s_data_act<0){
							printf("[DEBUG] ipv6 socket failed\n");
							free(vsockaddr);
							break;
						}
						if (connect(s_data_act, (struct sockaddr *)vsockaddr, (int)sizeof(struct sockaddr_in6)) != 0){
					 ...
					 printf("[DEBUG] client wants host to connect with %c MODE to %s ADDRESS at %s PORT", modePtr, hostPtr, portPtr);
